...
export async function listChats(userId: string): Promise<string[]> {
  ...
  const results = await db.select({ id: chatTable.id }).from(chatTable).where(eq(chatTable.userId, userId));
...
function toChat(row: DbChat): Chat {
  ...
  return { id: row.id, userId: row.userId, messages: JSON.parse(row.messages) as UIMessage[] }

}

function toDbChat(chat: Chat): DbChat {

  return {id: chat.id, userId: chat.userId, messages: JSON.stringify(chat.messages) }
